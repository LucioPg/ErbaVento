...
class EvInterface(mainwindow, QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.datePrivati = []
        self.datePulizie = []
        d = {"nome": "", "cognome": "", "telefono": None, "platform": "", "data arrivo": None, "data partenza": None,
             "numero ospiti": 1, "bambini": 0, "spese": {}, "colazione": False, "importo": 0, "lordo": 0, "tasse": 0,
             "netto": 0, "note": ""}
        self.infoModel = Od(d)
    ...
    def get_date(self, d):
        ...
        self.current_date = d.toString('ddd dd/MM/yyyy')
        print("selection changed", self.current_date)
        self.label_data.setText(self.current_date)
    ...
    def salvaInfo(self):
        ...
        if len(listaDisponibili):
            ...
            ff = g - 1
            # print("database = info 22222 : ",database[a][m][ff])
            ...
            if dal in listaDisponibili:
                ...
                for i in range(giorniPermanenza):
                    ...
                    # # database[a][m][g]['checkIn'] = self.infoModel.copy()
                    database[a][m][g]['checkOut'] = self.infoModelRedux.copy()
                    database[a][m][g]['checkOut']['data partenza'] = al.toString('dd MMM yyyy')
                    data = data.addDays(1)
    ...
    def leggiDatabase(self, database):
        ...
        numeriMesi = {'gen': 1, 'feb': 2, 'mar': 3, 'apr': 4, 'msg': 5, 'giu': 6, 'lug': 7, 'ago': 8, 'set': 9,
        ...
        for anno in database.keys():
            for mese in database[anno].keys():
                for giorno in database[anno][mese].keys():
                    ...
                    print(nome)
                    if database[anno][mese][giorno]['checkIn']['nome'] != '' or database[anno][mese][giorno]['checkIn'][
                        'cognome'] != '':
                        if database[anno][mese][giorno]['checkIn']['platform'] == 'Booking.com':
                            self.dateBooking.append(QtCore.QDate(int(anno), numeriMesi[mese], int(giorno)))
                        elif database[anno][mese][giorno]['checkIn']['platform'] == 'airBB':
                            self.dateAirbb.append(QtCore.QDate(int(anno), numeriMesi[mese], int(giorno)))
                        elif database[anno][mese][giorno]['checkIn']['platform'] == 'privato':
                            self.datePrivati.append(QtCore.QDate(int(anno), numeriMesi[mese], int(giorno)))
            ...
            for i in range(1, giorniPermanenza + 1):
                ...
                if info['nome'] != '' or info['cognome'] != '':
                    ...
                    if finoAl == data.toString('dd MMM yyyy') or finoAl is None:
                        ...
                        print("spiacente, casa non disponibile in questa data", data.toString('dd MMM yyyy'))
                        print(nome, "\t", cognome, "\n", "fino al: ", finoAl)
                        aval = False
...
class MyCalend(QtWidgets.QCalendarWidget):
    ...
    def __init__(self, dateListAir, dateListBooking, dateListPrivati, pulizieList, parent=None):
